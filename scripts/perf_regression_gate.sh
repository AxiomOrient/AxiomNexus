...
ROOT_DIR=""
ROOT_CREATED=false
DEFAULT_BIN="$(pwd)/target/debug/axiomsync-cli"
BIN="${AXIOMSYNC_BIN:-${DEFAULT_BIN}}"
...
Options:
  --root <path>                     AxiomSync root directory (default: temporary directory)
  --axiomsync-bin <path>              CLI binary path (default: target/debug/axiomsync-cli)
...
if [[ "$BIN_OVERRIDDEN" != "true" ]]; then
  cargo build -p axiomsync-cli >/dev/null
  BIN="${DEFAULT_BIN}"
fi
...
if [[ -z "$ROOT_DIR" ]]; then
  ROOT_DIR="$(mktemp -d /tmp/axiomsync-perf-root-XXXXXX)"
  ROOT_CREATED=true
fi

DATA_DIR="$(mktemp -d /tmp/axiomsync-perf-data-XXXXXX)"
