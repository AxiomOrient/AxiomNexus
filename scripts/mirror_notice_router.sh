...
GATE_JSON="${AXIOMSYNC_QUALITY_NOTICE_GATE_JSON:-${REPO_ROOT}/${QUALITY_REPORT_DIR}/mirror_notice_gate.json}"
OUTPUT_JSON="${AXIOMSYNC_QUALITY_NOTICE_ROUTER_JSON:-${REPO_ROOT}/${QUALITY_REPORT_DIR}/mirror_notice_router.json}"
...
Environment:
  AXIOMSYNC_QUALITY_REPORT_DIR         Base quality report dir for default paths (default: logs/quality)
  AXIOMSYNC_QUALITY_NOTICE_GATE_JSON   Override default gate JSON path
  AXIOMSYNC_QUALITY_NOTICE_ROUTER_JSON Override default router JSON path
EOU
}
...
selected_for_next="release.notice.ready"
...
if [[ "${status}" == "blocked" && "${reason}" == "post_notice_tag_missing" ]]; then
  selected_for_next="release.notice.await_tag"
  route_type="waiting"
  route_reason="waiting_for_post_notice_tag"
  action="Refresh notice gate snapshot until a post-notice release tag appears."
elif [[ "${status}" == "blocked" && ( "${reason}" == "strict_gate_failed" || "${reason}" == "strict_gate_skipped" ) ]]; then
  selected_for_next="release.notice.recover_strict_gate"
  route_type="actionable"
